...
class FingerTreeSpecWasowski extends FlatSpec with Checkers {
  ...
  def fingerTreeOfN[A] (n: Int, gen: Gen[A]) :Gen[FingerTree[A]] = {
    Gen.listOfN[A](n,gen).map(x => Digit.toTree[A](x))
  }
  ...
  // comprehensions).
  def fingerTree[A] (gen: Gen[A]) :Gen[FingerTree[A]] = Gen.choose(0,100).flatMap(
    x => fingerTreeOfN(x, gen))
  ...
  it should "produce a queue containing the inserted element" in {
    assert(Empty().addR(8).toList == List(8))
    assert(Empty().addR(8).addR(9).toList == List(8,9))
  }
 ...
 it should "be an identitity on trees" in check {
  ...
  it should "be ConsR(_,Nil) on Single" in {
    Single(42) match {
