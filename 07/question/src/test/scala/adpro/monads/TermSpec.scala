...
class  TermSpec extends FlatSpec {
  ...
  val answer3 = Div(Div(Div(Con(10), Con(2)), Con(1)), Con(1))
  val answer0 = Con(5)
 ...
 // Uncomment sections below as you proceed
   ...
   it should "give (5,3)" in {
     assert (StateEvaluatorWithMonads.eval (answer3).step (0) == (5,3))
   }
   
   it should "give (5,0)" in {
     assert (StateEvaluatorWithMonads.eval (answer0).step (0) == (5,0))
   }
