...
// Uncomment sections below as you proceed
 ...
 object OutputEvaluator {
   ...
   def eval (term :Term) :M[Int] = term match {
     ...
     case Div(t,u) => {
       val M (x, a) = eval(t)
       val M (y, b) = eval(u)
       M[Int] (x + y + line(Div(t,u)) (a / b), a / b)
     }
   }
 }
 ...
 object BasicEvaluatorWithMonads {
   ...
   def eval (term: Term) :M[Int] = term match {
     case Con (a) => M.unit (a)
     case Div (t,u) => {
       eval(t).flatMap(a => eval(u).flatMap(b => M.unit(a/b).map(r => r)))
     }
     //~ for {       
       //~ a <- eval (t)
       //~ b <- eval (u)
 ...
 object OutputEvaluatorWithMonads {
   ...
   // TODO: implement eval
   def eval (term :Term) :M[Int] = term match {
     case Con(a) => M(line(Con(a))(a), a) // new 'base' monad: ("eval (Con(8)) <= 8 \n"
     case Div(t,u) =>  {
       val x = eval(t)
       val y = eval(u)
...
//~ We have considered extending the ExceptionEvaluator to include the state
//~ aspect. It seems simpler to do it this way, because the basic control
//~ flow is already present and prepared to raise exceptions. We would extend the
//~ Return case class to contain the step function, which would allow us to count
//~ the number of divisions in the ExceptionEvaluator. The difficulty lies in
//~ modifying flatMap and map to inlcude the step function aspect. Furthermore,
//~ the eval function's second case (Div(t,u)) would need a similar modification.
